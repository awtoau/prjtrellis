...
def main():
    ...
    all_errors += nonrouting.check_enum_setting(cfg, "OSCH.STDBY", ["0", "1"],
                                  lambda x: get_substs(stdby=x), empty_bitfile)
    all_errors += nonrouting.check_enum_setting(cfg, "OSCH.MODE", ["NONE", "OSCH"],
                                  lambda x: get_substs(mode=x), empty_bitfile)
    all_errors += nonrouting.check_enum_setting(cfg, "OSCH.NOM_FREQ",
        ["{}".format(i) for i in [
